...
import { Send, Mail, Github, Linkedin, Twitter, MapPin, Instagram, Youtube, Dribbble, Globe, FileText } from "lucide-react";
    ...
    <main className="h-screen w-screen flex items-center justify-center md:pl-20 px-4 pb-20 md:pb-4 pt-4">
      <div className="w-full max-w-4xl grid grid-cols-1 md:grid-cols-5 gap-4 fade-up">
        ...
        <GlassCard className="md:col-span-2 rounded-2xl p-6 flex flex-col justify-between gap-6" depth={6}>
          <div>
            <p className="text-xs uppercase tracking-widest mb-1 font-syne" style={{ color: "hsl(var(--p))" }}>Get in touch</p>
          ...
          {sent ? (
            <div className="h-full flex flex-col items-center justify-center gap-3 text-center">
              <div className="w-14 h-14 rounded-2xl flex items-center justify-center text-3xl"
                style={{ background: "hsl(var(--p) / 0.1)" }}>✉️</div>
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4 h-full">
              <div className="grid grid-cols-2 gap-3">
                <div className="flex flex-col gap-1.5">
                  ...
                  <input required value={form.name} onChange={e => setForm(f => ({ ...f, name: e.target.value }))}
                    ...
                    style={{ background: "hsl(210 60% 6% / 0.8)", border: "1px solid hsl(var(--p) / 0.12)",
              ...
              <button type="submit"
                ...
                style={{ background: "linear-gradient(135deg, hsl(var(--p)), hsl(var(--p2)))",
                  color: "hsl(210 100% 4%)", boxShadow: "0 4px 24px hsl(var(--p) / 0.25)" }}>
